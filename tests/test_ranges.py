...
class TestUnboundedEnds:
    """``-inf`` and ``+inf`` have one spelling each.

    An inclusive unbounded end marks no point an exclusive one does not. Kept
    apart, the two would be unequal with neither sorting below the other, which
    ``functools.total_ordering`` reads as each being greater than the other.
    """
