...
def test_difference_equals_intersect_complement(
    ...
    this strategy does not draw; configured-policy mismatches (where both
    spellings raise) are covered by
    :func:`test_difference_matches_intersection_on_policy_mismatch`.
    """
...
@SETTINGS
def test_difference_matches_intersection_on_policy_mismatch(
    a: SpecifierSet, b: SpecifierSet
) -> None:
    """``a - b`` raises on a configured-policy mismatch exactly when ``a & ~b``
    does, so the two exclusion spellings agree on every input."""
    ra, rb = a.to_range(), b.to_range()
