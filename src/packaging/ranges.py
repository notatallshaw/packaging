...
class VersionRange:
    ...
    def from_bounds(
        ...
        *,
        ...
        Many ranges hold exactly the versions that lie between one pair of cuts;
        this returns the representative whose own bounds are those cuts. A caller
        that has worked out where the boundaries belong can state them directly
        instead of looking for a specifier set that happens to land there.
    ...
    def snap_bounds(self, versions: Iterable[Version | str]) -> VersionRange:
        ...
        A reduction operator: it restates self's bounds in terms of the given
        versions without changing what self says about any of them. Each finite
        segment end moves inward onto the outermost given version its segment
        contains; an end with no given version to land on is left alone, so an
        unbounded end and a segment holding none of the given versions both
        survive intact.

        Three properties hold for any range and any versions, and callers may
        rely on them:

        - Agreement. The result contains every given version self contains, and
          excludes every given version self excludes.
        - Direction. The result is a subset of self. Snapping only ever drops
          versions, so a stale or incomplete ``versions`` list costs precision
          and not soundness; a reducer that filled in the gaps between the given
          versions instead could render a version self forbids as allowed.
        - Idempotence. Snapping the result on the same versions returns an equal
          range.

        Set algebra leaves bounds at versions nobody released; snapping them onto
        real versions yields the range a human would write.
