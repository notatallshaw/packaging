...
class VersionRange:
    ...
    to share the same configured policy; set difference (``-``) does not.
    ...
    def difference(self, other: VersionRange) -> VersionRange:
        ...
        bounds-only exclusion, so its configured policy is ignored and the
        operands need not share one, unlike the :meth:`intersection` behind
        ``a & ~b``.
