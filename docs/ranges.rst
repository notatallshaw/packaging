...
It treats ``b`` purely as an exclusion, so unlike
:meth:`VersionRange.intersection` the operands need not share a configured
pre-release policy:

.. doctest::

    >>> strict = SpecifierSet(">=1.0", prereleases=False).to_range()
    >>> dropped = SpecifierSet(">=2.0", prereleases=True).to_range()
    >>> list((strict - dropped).filter(["1.5a1", "1.9"]))
    ['1.9']
    >>> strict & ~dropped
    Traceback (most recent call last):
        ...
    ValueError: Cannot combine VersionRange operands with different pre-release policies: False and True
